...
import { db } from '../db';
import { studyPlans, studyPlanModules, modules, userModules } from '../db/schema';
...
router.post('/', authenticateToken, validateRequest(createStudyPlanSchema), async (req, res) => {
  try {
    const userId = req.user!.id;
...
router.put('/:id', authenticateToken, validateRequest(updateStudyPlanSchema), async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
...
router.post('/:id/modules', authenticateToken, validateRequest(addModuleToStudyPlanSchema), async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
...
router.put('/:id/modules/:modulePool', authenticateToken, validateRequest(updateStudyPlanModuleSchema), async (req, res) => {
  try {
...
router.delete('/:id/modules/:modulePool', authenticateToken, async (req, res) => {
  try {
    const { id, modulePool } = req.params;
    const userId = req.user!.id;
...
router.get('/:id/summary', authenticateToken, async (req, res) => {
  try {
    ...
    const totalCredits = planModulesQuery.reduce((sum, item) => sum + item.module.credits, 0);
    
    const completedModules = planModulesQuery.filter(item => 
      item.userModule?.status === 'COMPLETED'
    ).length;
    ...
    const inProgressModules = planModulesQuery.filter(item => 
      item.userModule?.status === 'IN_PROGRESS'
    ).length;

    const plannedModules = planModulesQuery.filter(item => 
      !item.userModule || item.userModule.status === 'PLANNED'
    ).length;

    // Group by semester
    const semesterGroups = planModulesQuery.reduce((groups, item) => {
    ...
    res.json({
      ...
      data: {
        ...
        statistics: {
          ...
          creditProgress: totalCredits > 0 ? Math.round((completedCredits / totalCredits) * 100) : 0
        },
        semesterGroups
      }
    });
