...
const getRecommendationsSchema = {
  query: Joi.object({
    ...
    focusArea: Joi.string()
      .valid('INF', 'MAN', 'MK', 'GS', 'MAT', 'II', 'NAT', 'GEN')
      .optional(),
  ...
  async (req, res) => {
    try {
      ...
      const recommendations = calculateModuleRecommendations(
        ...
        userModulesData,
        {
          targetSemester: targetSemester as string,
          maxCredits: maxCredits as number,
...
router.post(
  '/study-plan',
  authenticateToken,
  validateRequest(optimizeStudyPlanSchema),
  async (req, res) => {
    try {
      const userId = req.user!.id;
      const {
      ...
      const studyPlan = await db
        ...
        .from(studyPlans)
        .where(
          and(eq(studyPlans.id, studyPlanId), eq(studyPlans.userId, userId))
        )
        .limit(1);
  ...
  return availableModules
    .map((module) => {
      ...
      return {
        ...
        recommendedSemester: calculateRecommendedSemester(
          module,
          userModules.length
        ),
      };
...
function optimizeStudyPlanDistribution(
  ...
  options: {
    ...
    prioritizeEasyModules: boolean;
  }
) {
  const { semesterCount, maxCreditsPerSemester, prioritizeEasyModules } =
    options;

  // Sort modules intelligently
  const sortedModules = modulesList.sort(
    (a: typeof modules.$inferSelect, b: typeof modules.$inferSelect) => {
      if (prioritizeEasyModules) return a.credits - b.credits;

      // Foundation first, then by complexity
      if (a.pool === 'Orientierungsphase' && b.pool !== 'Orientierungsphase')
        return -1;
      if (b.pool === 'Orientierungsphase' && a.pool !== 'Orientierungsphase')
        return 1;
  ...
  for (const module of sortedModules) {
    ...
    for (let sem = 1; sem <= semesterCount && !placed; sem++) {
      ...
      const currentCredits = distribution[semKey].reduce(
...
function calculateAverageGrade(
  userModules: Array<{ grade: number | null; status: string }>
): number | null {
...
function inferPrerequisites(module: typeof modules.$inferSelect): string[] {
  ...
  if (
    module.name.toLowerCase().includes('advanced') ||
    module.name.includes('Machine Learning')
  ) {
...
function calculateRecommendedSemester(
  module: typeof modules.$inferSelect,
  userProgress: number
): number {
  if (module.pool === 'Orientierungsphase') return 1;
  if (module.pool === 'IT Vertiefung')
    return Math.max(2, Math.ceil(userProgress / 3));
